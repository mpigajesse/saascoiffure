...
export default function SettingsPage() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      await apiClient.patch('/api/v1/auth/users/update-profile/', data);
  ...
  const handleSaveSalon = async () => {
    ...
    try {
      ...
      await apiClient.patch('/api/v1/salons/update-my-salon/', data);

      // Forcer le rafraîchissement du salon via /auth/me/ pour garantir la cohérence
      if (refreshSalon) {
        await refreshSalon();
      }
      // Rafraîchir l'utilisateur aussi pour mettre à jour le contexte global
      if (refreshUser) {
        await refreshUser();
      }
  ...
  const handleCurrencyChange = async (newCurrency: string) => {
    ...
    try {
      ...
      const response = await apiClient.patch('/api/v1/salons/update-my-salon/', {
            ...
            <form ref={salonFormRef}>
              ...
              <div className="bg-card border-2 border-border p-6 space-y-6">
                <div className="flex items-center gap-3 pb-4 border-b border-border">
                  ...
                  <h2 className="text-lg font-bold">Horaires d'ouverture (par jour)</h2>
                </div>
                {(() => {
                  const { openingHours, loading, error, setOpeningHours } = useOpeningHours();
                  const [editState, setEditState] = useState<{ [id: number]: Partial<import('@/types').SalonOpeningHour> }>({});
                  ...
                  const handleSave = async (id: number) => {
                    setSavingId(id);
                    const hour = openingHours.find(h => h.id === id);
                    if (!hour) return;
                    const update = { ...hour, ...editState[id] };
                    try {
                      await apiClient.patch(`/api/v1/opening-hours/${id}/`, update);
                      setOpeningHours(hours => hours.map(h => h.id === id ? { ...h, ...editState[id] } : h));
                      setEditState(prev => ({ ...prev, [id]: {} }));
                      toast({
                        title: "Horaire mis à jour",
                        description: "Les horaires d'ouverture ont été enregistrés avec succès.",
                      });
                    } catch (e) {
                      console.error(e);
                      toast({
                        title: "Erreur",
                        description: "Une erreur est survenue lors de la sauvegarde de l'horaire.",
                        variant: "destructive",
                      });
                    } finally {
                      setSavingId(null);
                  ...
                  if (loading) return <div className="text-muted-foreground">Chargement des horaires...</div>;
                  if (error) return <div className="text-destructive">Erreur: {error}</div>;
                  if (!Array.isArray(openingHours)) return <div className="text-destructive">Erreur: données horaires invalides</div>;
                  if (!openingHours.length) return <div className="text-muted-foreground">Aucun horaire configuré.</div>;
                  return (
                    <div className="space-y-2">
                      {openingHours.map(hour => (
                        <div key={hour.id} className="flex items-center gap-4 border-b border-border py-2">
                          <span className="w-24 font-medium">{hour.day_of_week_display}</span>
                          <input type="checkbox" checked={editState[hour.id] && 'is_closed' in editState[hour.id] ? !editState[hour.id].is_closed : !hour.is_closed} onChange={e => handleChange(hour.id, 'is_closed', !e.target.checked)} />
                          {(editState[hour.id] && 'is_closed' in editState[hour.id] ? editState[hour.id].is_closed : hour.is_closed) ? (
                            <span className="text-destructive">Fermé</span>
                          ) : (
                            <>
                              <label>Ouverture: <input type="time" value={editState[hour.id]?.open_time ?? hour.open_time} onChange={e => handleChange(hour.id, 'open_time', e.target.value)} className="font-mono border rounded px-1" /></label>
                              <label>Fermeture: <input type="time" value={editState[hour.id]?.close_time ?? hour.close_time} onChange={e => handleChange(hour.id, 'close_time', e.target.value)} className="font-mono border rounded px-1" /></label>
                            </>
                          )}
                          <button className="ml-2 px-2 py-1 bg-primary text-white rounded disabled:opacity-50" disabled={savingId === hour.id} onClick={() => handleSave(hour.id)}>
                            {savingId === hour.id ? '...' : 'Enregistrer'}
                          </button>
                        </div>
                      ))}
                    </div>
